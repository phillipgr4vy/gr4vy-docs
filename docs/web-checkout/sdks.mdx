...
The first argument of the `NewGr4vyClient` represents the unique ID of your
Gr4vy instance. Refer to the integrations panel of your dashboard for more
details.
...
This assumes the key you created in step 1 is kept in a file called
`private_key.pem` that is kept in the same folder next to the code. You could
store this key on disk, in an environment variable, or in a secure vault.

The first argument of the `Gr4vyConfig` represents the unique ID of your Gr4vy
instance. Refer to the integrations panel of your dashboard for more details.
...
This token can now be served in your checkout page, where it will be used by
Gr4vy Embed.

:::info Manual Token Generation

If you would prefer to generate the client token without using a Gr4vy
server-side SDK, please read our guide [here](/authentication#manual-jwt).
