...
A buyer can be added with a simple call to the [`POST /buyers`][api] API. The
call accepts an optional `external_identifier` (e.g. the user's email, username,
or a database ID) and an optional `display_name` which will be shown in the
admin panel.
...
[transactions]: /guides/buyers/associate
