...
sidebar_label: Token transaction
---
...
The call requires an `amount`, `currency`, and a `payment_method` with a
`token`. This token can represent a card, a PayPal account, or any other payment
method.
