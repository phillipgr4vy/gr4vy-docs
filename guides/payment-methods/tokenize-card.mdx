...
A card can be stored by calling the [`POST /payment-methods`][api] API. The call
requires a `method` and the card `number` and an optional `security_code` and
`expiration_date` for the card details.
