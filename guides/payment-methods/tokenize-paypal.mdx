...
API. The call requires a `method` and an optional `redirect_url`. Additionally,
the API call accepts a `buyer_id` or `buyer_external_identifier` which can be
used to associate a card to a previously registered [buyer].
...
The first step is to initialize a new PayPal payment method. An optional
`redirect_url` can be provided to redirect the user back to your application
after they have approved the application. In most cases you will want to provide
this URL because otherwise the user will not be redirected back to your
application.
...
```json title="POST /payment-methods"
{
  ...
  "details": {
    "authorization_url": "https://redirect.gr4vy.com/....."
  },
...
```
http://localhost:3000?complete?payment_method_id=77a76f7e-d2de-4bbc-ada9-d6a0015e6bd5
```

The `payment_method_id` query parameter represents the ID of the payment method
that this PayPal account has been registered as.

## Step 3. Confirm the authorization

Finally, make an API call to confirm the PayPal payment has been fully
authorized.
...
```json title="POST /payment-methods/77a76f7e-d2de-4bbc-ada9-d6a0015e6bd5/authorize"
{
  "type": "payment-method",
  "id": "77a76f7e-d2de-4bbc-ada9-d6a0015e6bd5",
  "method": "paypal",
