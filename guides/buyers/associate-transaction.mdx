---
title: Associate a transaction to a buyer
sidebar_label: Associate transaction
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";

There are a few ways to associate a transaction to a buyer.

## Associate by `buyer_id`

The easiest way to associate a buyer to a transaction is to pass along a
`buyer_id` when creating a transaction.

This will find a buyer with that `id` and associate the transaction to it. If a
buyer with that `id` could not be found it's ignored.

<Tabs
  groupId="language"
  defaultValue="curl"
  values={[
    { label: 'cURL', value: 'curl', },
    { label: 'Node', value: 'node', },
  ]}
>
  <TabItem value="curl">

```bash {7}
curl -i -X POST "https://***.api.gr4vy.com/transactions" \
    -H "Authorization: Bearer [JWT_TOKEN]>" \
    -H "Content-Type: application/json" \
    -d '{
      "amount": 1299,
      "currency": "USD",
      "buyer_id": "fe26475d-ec3e-4884-9553-f7356683f7f9",
      "payment_method_type": "card",
      "payment_method": {
        "number": "4111111111111111",
        "expiration_date": "11/25",
        "security_code": "123"
      }
    }'
```

  </TabItem>
  <TabItem value="node">

```js {12}
const fetch = require("node-fetch");

fetch("https://***.api.gr4vy.com/transactions", {
  method: "POST",
  headers: {
    Authorization: "Bearer [JWT_TOKEN]>",
    "Content-Type": "application/json",
  },
  body: JSON.stringify({
    amount: 1299,
    currency: "USD",
    buyer_id: "fe26475d-ec3e-4884-9553-f7356683f7f9",
    payment_method_type: "card",
    payment_method: {
      number: "4111111111111111",
      expiration_date: "11/25",
      security_code: "123",
    },
  }),
});
```

  </TabItem>
</Tabs>

## Associate by `buyer_external_identifier`

Alternatively, a transaction can be associated to a buyer by it's
`external_identifier`. If a buyer with that `external_identifier` could not be
found it's ignored.

<Tabs
  groupId="language"
  defaultValue="curl"
  values={[
    { label: 'cURL', value: 'curl', },
    { label: 'Node', value: 'node', },
  ]}
>
  <TabItem value="curl">

```bash {7}
curl -i -X POST "https://***.api.gr4vy.com/transactions" \
    -H "Authorization: Bearer [JWT_TOKEN]>" \
    -H "Content-Type: application/json" \
    -d '{
      "amount": 1299,
      "currency": "USD",
      "buyer_external_identifier": "412231123",
      "payment_method_type": "card",
      "payment_method": {
        "number": "4111111111111111",
        "expiration_date": "11/25",
        "security_code": "123"
      }
    }'
```

  </TabItem>
  <TabItem value="node">

```js {12}
const fetch = require("node-fetch");

fetch("https://***.api.gr4vy.com/transactions", {
  method: "POST",
  headers: {
    Authorization: "Bearer [JWT_TOKEN]>",
    "Content-Type": "application/json",
  },
  body: JSON.stringify({
    amount: 1299,
    currency: "USD",
    buyer_external_identifier: "412231123",
    payment_method_type: "card",
    payment_method: {
      number: "4111111111111111",
      expiration_date: "11/25",
      security_code: "123",
    },
  }),
});
```

  </TabItem>
</Tabs>
