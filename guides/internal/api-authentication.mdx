...
```bash
curl -i -X POST "https://api.{gid}.gr4vy.app/auth/sessions" \
...
```bash
curl -i -X PUT "https://api.{gid}.gr4vy.app/auth/sessions" \
...
```bash
curl -i -X POST "https://api.{gid}.gr4vy.app/api-key-pairs" \
