...
| `{resources}.write` | Allows write-access to a type or resource. For example, `buyers.write` enabled write-access for buyers data. This does not allow read access. |
| `session_refresh`   | Used by the refresh token, allowing it to be used to refresh the access token.                                                                |
| `embed`             | A scope that represents all the acces needed by Gr4vy Embed.                                                                                  |
| `cluster_admin`     | A scope that represents all the acces needed by Gr4vy Embed.                                                                                  |
| `server`            | A scope that represents all APIs that can be accessed server-to-server.                                                                       |
